...
const productForm = ref({
  id: null, name: '', categoryId: 1, price: 0, unit: 'Kg', stockQty: 0, imageUrl: '', isActive: true
})
...
const handleImageError = (e) => e.target.src = 'https://placehold.co/150x150/f9fafb/a1a1aa?text=No+Image'

const fetchProducts = async () => {
  isLoading.value = true
  try {
    const res = await axiosInstance.get('/products', { params: { page: 0, size: 50, sort: 'desc' } })
    let finalData = res.data !== undefined ? res.data : res
    if (finalData && finalData.data) finalData = finalData.data
    if (finalData && finalData.content) finalData = finalData.content
    products.value = Array.isArray(finalData) ? finalData : []
  } catch (error) {
...
const deleteProduct = async (id) => {
  ...
  try {
    ...
    toast.error('Không thể xóa sản phẩm. Có thể do sản phẩm đang có đơn hàng.')
  } finally {
    isSaving.value = false
  }
}
  ...
  <div class="space-y-8 relative">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
      <div class="relative flex-1 w-full max-w-md">
        <input v-model="searchQuery" type="text" placeholder="Tìm kiếm sản phẩm theo tên..." class="w-full bg-gray-50 border border-gray-200 rounded-2xl pl-12 pr-4 py-3.5 text-sm focus:outline-none focus:border-[#82ae46] focus:ring-1 focus:ring-[#82ae46]" />
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-100 text-sm font-medium text-gray-700">
            <tr v-if="filteredProducts.length === 0 && !isLoading"><td colspan="7" class="py-12 text-center text-gray-400">Không có sản phẩm nào.</td></tr>
            <tr v-for="product in filteredProducts" :key="product.id" class="hover:bg-gray-50/50 transition-colors">
              ...
              <td class="py-4 px-6"><span class="font-bold text-gray-900 line-clamp-1">{{ product.name }}</span><span class="text-xs text-gray-400 block mt-0.5">ID: #{{ product.id }}</span></td>
              <td class="py-4 px-6 text-center"><span class="bg-gray-100 text-gray-600 text-xs px-3 py-1 rounded-full font-bold uppercase tracking-wider">{{ product.categoryName || `DM ${product.categoryId}` || 'Khác' }}</span></td>
              <td class="py-4 px-6 text-right font-black text-gray-900">{{ formatPrice(product.price) }} <span class="text-xs font-normal text-gray-400">/ {{ product.unit }}</span></td>
              <td class="py-4 px-6 text-center font-bold"><span :class="product.stockQty > 0 ? 'text-gray-800' : 'text-red-500 font-black'">{{ product.stockQty > 0 ? `${product.stockQty} ${product.unit}` : 'Hết hàng' }}</span></td>
              <td class="py-4 px-6 text-center"><span class="inline-flex px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider" :class="product.isActive ? 'bg-green-50 text-green-600 border border-green-200' : 'bg-red-50 text-red-600 border border-red-200'">{{ product.isActive ? 'Kinh doanh' : 'Ngừng bán' }}</span></td>
              <td class="py-4 px-6">
                <div class="flex items-center justify-center gap-2">
                  <button @click="openEditModal(product)" class="p-2 text-blue-500 hover:bg-blue-50 rounded-xl transition-all" title="Sửa"><Edit2 :size="16" /></button>
                  <button @click="deleteProduct(product.id)" class="p-2 text-red-500 hover:bg-red-50 rounded-xl transition-all" title="Xóa"><Trash2 :size="16" /></button>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div class="bg-white w-full max-w-2xl rounded-[30px] p-8 relative">
        <button @click="showModal = false" class="absolute top-6 right-6 text-gray-400 hover:text-gray-900 transition-colors"><X class="w-6 h-6" /></button>
        <h3 class="text-2xl font-black text-gray-900 mb-6 uppercase tracking-wider">{{ isEditing ? 'Cập nhật sản phẩm' : 'Thêm sản phẩm mới' }}</h3>
        <form @submit.prevent="saveProduct" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
              ...
              <input v-model="productForm.name" required type="text" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#82ae46]" />
            </div>
            <div>
            ...
            <div>
              <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Đơn vị</label>
            ...
            <div>
              ...
              <input v-model.number="productForm.categoryId" required type="number" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#82ae46]" />
            </div>
            <div class="md:col-span-2">
              <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Link Ảnh (URL)</label>
              <input v-model="productForm.imageUrl" type="url" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#82ae46]" />
            </div>
          </div>
          <div class="pt-2 flex items-center gap-3">
            <input v-model="productForm.isActive" type="checkbox" id="isActive" class="w-5 h-5 text-[#82ae46] rounded focus:ring-[#82ae46]" />
            <label for="isActive" class="text-sm font-bold text-gray-700 cursor-pointer">Đang kinh doanh</label>
          </div>
          <div class="pt-6 border-t border-gray-100 flex gap-4">
            ...
            <button type="button" @click="showModal = false" class="px-8 bg-gray-100 hover:bg-gray-200 text-gray-700 py-4 rounded-xl font-bold uppercase tracking-widest transition-colors">Hủy</button>
          </div>
        </form>
      </div>
    </div>
